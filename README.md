...
`app/lib/shopify.server.ts` と `app/components/PrintAIze.tsx` で、実際の商品のバリアントIDに変更してください：
...
├── app/
│   ├── components/
│   │   └── PrintAIze.tsx    # メインカスタマイザーコンポーネント
│   ├── lib/
...
`app/routes/_index.tsx` と `app/components/PrintAIze.tsx` でスタイルを変更できます。
